...
describe('MongoStorageProvider', () => {
  ...
  beforeEach(async () => {
    storage = new MongoStorageProvider(connectionString, 'test-taskdriver', false); // Disable transactions for testing
  ...
  describe('Health Check', () => {
    it('should return healthy status', async () => {
      const result = await storage.healthCheck();
      expect(result.healthy).toBe(true);
      expect(result.message).toContain('MongoDB connection is healthy');
    });
  });
